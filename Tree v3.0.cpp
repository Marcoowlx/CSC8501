...
//比较
 int Comparable::compare_to(const Comparable& rhs) {
	 return 0;
}

 int Comparable::get(const Comparable& rhs) {
	 return c->value;
 }



 class Obj : public Comparable {
 protected:
	 Obj();
	 ~Obj();
 ...
 int Obj::compare_to(const Comparable& rhs) {
	 return 0;
//	 if ((c->get) > (rhs.get))	return 1;
//	 if ((c->get) == (rhs.get)) return 0;
//	 if ((c->get) < (rhs.get)) return -1;
 }
